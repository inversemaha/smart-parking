    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ __('Edit Vehicle') }}</h1>
                    <p class="text-gray-600 mt-1">{{ __('Update your vehicle information') }}</p>
                </div>
        ...
        <div class="bg-white shadow rounded-lg p-6">
            <form action="{{ route('visitor.vehicles.update', $vehicle) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <select name="color" id="color"
                                ...
                                <option value="White" {{ old('color', $vehicle->color) == 'White' ? 'selected' : '' }}>{{ __('White') }}</option>
                                <option value="Black" {{ old('color', $vehicle->color) == 'Black' ? 'selected' : '' }}>{{ __('Black') }}</option>
                                <option value="Silver" {{ old('color', $vehicle->color) == 'Silver' ? 'selected' : '' }}>{{ __('Silver') }}</option>
                                <option value="Gray" {{ old('color', $vehicle->color) == 'Gray' ? 'selected' : '' }}>{{ __('Gray') }}</option>
                                <option value="Red" {{ old('color', $vehicle->color) == 'Red' ? 'selected' : '' }}>{{ __('Red') }}</option>
                                <option value="Blue" {{ old('color', $vehicle->color) == 'Blue' ? 'selected' : '' }}>{{ __('Blue') }}</option>
                                <option value="Green" {{ old('color', $vehicle->color) == 'Green' ? 'selected' : '' }}>{{ __('Green') }}</option>
                                <option value="Yellow" {{ old('color', $vehicle->color) == 'Yellow' ? 'selected' : '' }}>{{ __('Yellow') }}</option>
                                <option value="Brown" {{ old('color', $vehicle->color) == 'Brown' ? 'selected' : '' }}>{{ __('Brown') }}</option>
                                <option value="Other" {{ old('color', $vehicle->color) == 'Other' ? 'selected' : '' }}>{{ __('Other') }}</option>
                            </select>
                        </div>
                        @error('color')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <select name="vehicle_type" id="vehicle_type"
                                ...
                                <option value="car" {{ old('vehicle_type', $vehicle->vehicle_type) == 'car' ? 'selected' : '' }}>{{ __('Car') }}</option>
                                <option value="motorcycle" {{ old('vehicle_type', $vehicle->vehicle_type) == 'motorcycle' ? 'selected' : '' }}>{{ __('Motorcycle') }}</option>
                                <option value="suv" {{ old('vehicle_type', $vehicle->vehicle_type) == 'suv' ? 'selected' : '' }}>{{ __('SUV') }}</option>
                                <option value="truck" {{ old('vehicle_type', $vehicle->vehicle_type) == 'truck' ? 'selected' : '' }}>{{ __('Truck') }}</option>
                                <option value="van" {{ old('vehicle_type', $vehicle->vehicle_type) == 'van' ? 'selected' : '' }}>{{ __('Van') }}</option>
                            </select>
                        </div>
                        @error('vehicle_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea name="notes" id="notes" rows="3"
                                      ...
                                      placeholder="{{ __('Any additional notes about your vehicle') }}">{{ old('notes', $vehicle->notes) }}</textarea>
                        </div>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input type="checkbox" name="is_default" id="is_default" value="1"
                                   {{ old('is_default', $vehicle->is_default) ? 'checked' : '' }}
