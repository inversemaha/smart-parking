...
<div class="top-bar">
    ...
    <div class="intro-x dropdown mr-3 sm:mr-6">
        <div class="dropdown-toggle btn btn-sm px-2 box flex items-center gap-2" role="button" aria-expanded="false" data-tw-toggle="dropdown">
            ...
            <span class="hidden sm:inline text-xs font-medium uppercase"><?php echo e(app()->getLocale()); ?></span>
        </div>
        <div class="dropdown-menu w-40">
            <ul class="dropdown-content">
                <li>
                    <form action="<?php echo e(route('language.switch', 'en')); ?>" method="POST" class="inline">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="dropdown-item w-full text-left <?php echo e(app()->getLocale() === 'en' ? 'bg-primary/10 text-primary' : ''); ?>">
                            <i data-lucide="check" class="w-4 h-4 mr-2 <?php echo e(app()->getLocale() === 'en' ? 'inline' : 'hidden'); ?>"></i>
                            <span>English</span>
                        </button>
                    </form>
                </li>
                <li>
                    <form action="<?php echo e(route('language.switch', 'bn')); ?>" method="POST" class="inline">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="dropdown-item w-full text-left <?php echo e(app()->getLocale() === 'bn' ? 'bg-primary/10 text-primary' : ''); ?>">
                            <i data-lucide="check" class="w-4 h-4 mr-2 <?php echo e(app()->getLocale() === 'bn' ? 'inline' : 'hidden'); ?>"></i>
